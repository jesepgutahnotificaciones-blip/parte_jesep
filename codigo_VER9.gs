 ...
 * Google Apps Script Backend - VERSION V8
 * 
 * CAMBIOS V8:
 * - handleGetData ahora acepta parametro 'sheet' para leer hojas especificas
 * - Si no hay 'sheet', usa 'dependency' como nombre de hoja (compatibilidad)
 * - Lee rango D2:H1000 de las hojas de turno
 * ============================================================
 */
  ...
  try {
    ...
    switch (action) {
      ...
      case 'verifyDepKey':
        ...
        break;
      case 'getData':
        // V8: usa 'sheet' si viene, sino 'dependency'
...
function handleVerifyDepKey(username, depKey, dependencia) {
  ...
  console.log('username:', username, 'depKey:', depKey, 'dependencia:', dependencia);
...
// ============================================================
// GET DATA - V8: Lee D2:H1000 de la hoja especificada
// ============================================================
function handleGetData(sheetName) {
  console.log('=== handleGetData V8 === sheetName=' + sheetName);
  ...
  try {
    ...
    if (!sheet) {
      console.log('ERROR: Hoja no encontrada:', sheetName);
      return { success: false, error: 'Hoja no encontrada: ' + sheetName };
    }

    // Leer D2:H1000 (5 columnas: Grado, Nombres, Turno, Novedad, Observacion)
    const lastRow = sheet.getLastRow();
    const rowsToRead = Math.min(1000, Math.max(0, lastRow - 1));
    
    if (rowsToRead === 0) {
      console.log('Hoja vacia');
    ...
    values.forEach(function (row, i) {
      const grado = row[0];      // D
      const nombres = row[1];    // E
      const turno = row[2];      // F
      const novedad = row[3];    // G
      const observacion = row[4];// H
      
      // Solo incluir filas que tengan al menos grado o nombre
      if ((grado && grado.toString().trim()) || (nombres && nombres.toString().trim())) {
        result.push({
          ...
          turno: turno ? turno.toString().trim().toUpperCase() : '',
          novedad: novedad ? novedad.toString().trim() : '',
          observacion: observacion ? observacion.toString().trim() : ''
        });
      }
    });
...
function handleSaveData(dependency, params) {
  ...
  try {
    ...
    const lastRow = sheet.getLastRow();
    if (row > lastRow) return { success: false, error: 'Fila ' + row + ' no existe. Ultima: ' + lastRow };
